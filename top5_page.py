...
def top5_sayfasi_olustur(page: ft.Page, geri_don_fonksiyonu):
    ...
    def ai_veri_uret(zaman_dilimi):
        # 100 coin arasından rastgele 10 tanesini radarımıza takılmış gibi seçiyoruz
        secilenler = random.sample(COINLER, 10)
        ...
        for i in range(5):
            ...
            yukselenler.append({
                "coin": y_coin, "fiyat": COIN_FIYATLARI[y_coin] * random.uniform(0.9, 1.1), "yuzde": round(random.uniform(1.5 * carpan, 5.0 * carpan), 2),
                "hacim": f"${round(random.uniform(10 * carpan, 500 * carpan), 1)}M"
            })
            dusenler.append({
                "coin": d_coin, "fiyat": COIN_FIYATLARI[d_coin] * random.uniform(0.9, 1.1), "yuzde": round(random.uniform(-5.0 * carpan, -1.5 * carpan), 2),
    ...
    def liste_karti_uret(veri, is_gainer):
        ...
        return ft.Container(
            content=ft.Row([
                ft.Column([ft.Text(veri["coin"], weight="900", size=16, color="white"), ft.Text(fiyat_metni, color="#A0A0A5", size=12)], expand=1),
    ...
    zaman_dropdown = ft.Dropdown(options=[ft.dropdown.Option("15 Dakikalık"), ft.dropdown.Option("1 Saatlik"), ft.dropdown.Option("1 Günlük")], value="1 Saatlik", width=180, bgcolor="#0A0A0E", border_color="#1A1A24", color="white")
    ...
    ana_icerik = ft.Container(
        content=ft.Column([
            ...
            ft.Divider(color="#1A1A24"), ft.Container(height=10),
            ft.Row([
    ...
    def otomatik_yenile():
        while sayfa_aktif[0]:
            time.sleep(5)
            if sayfa_aktif[0]: sayfalari_guncelle(zaman_dropdown.value)
    ...
    return ft.Container(content=ft.Column([ft.Row([ft.TextButton(content=ft.Text("⬅️ Panoya Dön", color="#00ffcc"), on_click=lambda e: guvenli_cikis())]), ana_icerik], expand=True), bgcolor="#030304", padding=10, expand=True)